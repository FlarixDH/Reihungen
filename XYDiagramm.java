...
{      
    ...
    public void ladeTabelle(String name) {
        ...
        if (csv != null  && csv.getColumnCount()==2) {
            ...
            x_wert = new double[csv.getRowCount()];
            y_wert = new double[csv.getRowCount()];
            ...
            for (int i = 0; i < x_wert.length; i++) {
                ...
                x_wert[i] = csv.getDouble(i, "Time (s)");
                y_wert[i] = csv.getDouble(i, "Sound pressure level (dB)");
            }
        }
    }
    ...
    public void zeichneBalken() {
        ...
        if (x_wert != null) {
            for (int i = 0; i< x_wert.length; i++) {
                ...
                if (x_wert[i]>=0) rect(120, 25+i*15, (float)y_wert[i]+1, 13);

                // Beschriftung
                fill(255,255,255);
                text(""+y_wert[i], 2, 35+i*15);
                text(""+x_wert[i], 70, 35+i*15);
            }
        }
    }

    //public void zeichneZemmel() {
